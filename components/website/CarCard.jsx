...
export default function CarCard({ view }) {
  return (
    <div className={` rounded-xl overflow-hidden border border-gray-200 shadow-sm  group flex ${view === "list" ? "flex-row" : "flex-col max-w-full"}`}>
      <Swiper
        modules={[Navigation]}
        navigation={{ prevEl: ".custom-prev", nextEl: ".custom-next" }}
        className={` ${view === "list" ? "max-w-96 md:max-w-80 xl:max-w-96 h-auto" : "w-full h-64"}`}
      >
        <SwiperSlide>
          <img
    ...
    ❯
      ...
      <div className="px-4 py-3  w-full">
        <div className="flex flex-col justify-start mb-2">
         ...
         <h2 className={` font-semibold uppercase text-black mb-2 ${view === "list" ? "text-2xl" : "text-lg"}`}>2023 Toyota RAV4 </h2>
         </div>
        ...
        <div className="grid grid-cols-2 gap-2 font-medium mb-3 text-base text-black">
          <div><span className="font-light">Fuel:</span> Hybrid</div>
          <div><span className="font-light">Transmission:</span> Auto</div>
          <div><span className="font-light">Engine:</span> 2.5L</div>
          <div><span className="font-light">Mileage:</span> 166,000</div>
        </div>
        <p className="text-sm text-gray-500 mb-4">
          Lorem ipsum dolor sit amet cons 
        </p>
        <div className="relative flex justify-between items-center">
          <div className={` ${view === "list" ? "flex items-center gap-2" : "flex items-center gap-2"}`}>
          <img src="https://static.autotempest.com/prod/build/main/img/at-logos/at-logo-500.a9d7fdcf.png" alt="" className="w-32" />
