                        ...
                        <motion.div key="contact" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.3 }} className="max-w-2xl mx-auto">
                            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-4">
                                {phones.map((phoneObj, idx) => {
                                    ...
                                    return (
                                        <motion.a key={idx} variants={itemVariants} whileHover={{ scale: 1.02, x: 8 }} whileTap={{ scale: 0.98 }} href={`tel:${String(phone).replace(/[^+\d]/g, "")}`} className="flex items-center gap-4 p-5 bg-white dark:bg-gray-800 rounded-xl border-2 border-gray-200 dark:border-gray-700 hover:border-green-500 dark:hover:border-green-400 transition-all shadow-md hover:shadow-xl group">
